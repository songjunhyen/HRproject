...
> 기본 순서: **DB → DAO → Service → API/JSP → 검증**

| 단계 | 내용 | 진행 상태 |
|------|------|------------|
| **application.yml (9005 / 뷰리졸버 / DB)** | 포트 및 DB 연결 설정 | ✅ (2025.10.23 완료) |
| **HrmsApplication.java 실행 확인** | 서버 구동 및 기본 메시지 출력 | ✅ (2025.10.23 완료) |
| **DB 테이블 및 속성 구조 결정** | ERD 및 컬럼 정의 | ✅ (2025.10.23 완료) |
| **MyBatis 설정 및 Mapper 연결** | MapperScan, XML 경로 설정, 연결 테스트 | ✅ (2025.10.23 완료)  |
| **기능 구현** | DAO / Service / REST / JSP / 검증 / 테스트 / 전역 설정 | 🔜 이후 단계 |

---

### 🔧 기능 구현 세부 단계

- **DAO**: CRUD 인터페이스 + XML 매핑  
- **Service**: 비밀번호 해시·검증 및 비즈니스 규칙  
- **REST**: GET / POST / PUT / DELETE 구현  
- **JSP**: `/admin/employees.jsp` 목록·등록 화면  
- **검증**: DTO + `@Valid` + 에러 메시지 처리  
- **테스트**: 단위·통합 테스트 작성  
- **전역 설정**: 인터셉터, 로깅, 공통 예외 처리  
